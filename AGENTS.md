...
Every edit to `pyproject.toml`, including a version-only bump, requires `uv lock` and `uv lock --check`. The lock pins the project's own version; stale metadata breaks `uv sync --locked` before tests execute. Keep the Python, desktop and lockfile versions consistent.

Tag releases only after green CI on the bump commit, not its parent. In this fork `origin` points to NousResearch/hermes-agent; release automation must resolve the downstream remote and changelog URL to SlowGreek/costas-code. Never publish fork tags to upstream or rewrite a published ref without explicit user authorization.
...
## Downstream release engineering notes
