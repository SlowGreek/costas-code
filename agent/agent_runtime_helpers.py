...
AGENT_RUNTIME_POST_HOOK_TOOL_NAMES = frozenset(
    {"todo", "session_search", "memory", "clarify", "read_terminal", "read_preview", "desktop_preview", "drive_preview", "annotate_preview", "read_window_below", "setup_mcp", "tour", "delegate_task", "workflow"}
)
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    if function_name == "todo":
        ...
        # Agent-level, like delegate_task: the workflow runtime needs the
        # parent agent to derive an explicit spawn depth (see
        # AIAgent._dispatch_workflow).
        def _execute(next_args: dict) -> Any:
            return _finish_agent_tool(agent._dispatch_workflow(next_args), next_args)
    else:
        def _execute(next_args: dict) -> Any:
