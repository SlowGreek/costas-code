...
AGENT_RUNTIME_POST_HOOK_TOOL_NAMES = frozenset({
    ...
    "drive_preview", "annotate_preview", "read_window_below", "setup_mcp", "gui_tour", "delegate_task", "read_preview", "workflow",
})
...
def _merge_consecutive_users(messages: List[Dict]) -> Tuple[List[Dict], int]:
    ...
    for msg in messages:
        ...
        if (
            ...
            and not any(isinstance(row.get("display_metadata"), dict)
                        and row["display_metadata"].get("steering") for row in (prev, msg))
