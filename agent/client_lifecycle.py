    ...
    def _checkout_request_slot(self, slot_attr: str, key: Any) -> tuple:
        ...
        with self._openai_client_lock():
            ...
            if cached is None or cache["in_use"]:
                return None, None
            if not cache["poisoned"] and (self._request_client_kwargs_match(cache["key"], key) if slot_attr == _OPENAI_SLOT else cache["key"] == key) and not self._is_openai_client_closed(cached):
                cache["in_use"] = True
                return cached, None
