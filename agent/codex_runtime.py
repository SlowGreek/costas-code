...
class _CodexResponseAssembler:
    ...
    def _on_function_call(self, event: Any, event_type: str) -> None:
        ...
        if pending is None:
            ...
            matches = [p for p in self.pending_function_calls.values() if p["output_index"] == index] if index is not None else []
    ...
    def _on_item_done(self, event: Any, event_type: str) -> None:
        ...
        if _event_field(done_item, "type", "") == "function_call":
            call_id = _event_field(done_item, "call_id", None)
            matches = [key for key, pending in self.pending_function_calls.items()
                       if call_id and _event_field(pending["item"], "call_id", None) == call_id]
            if len(matches) == 1:
                pending_id = matches[0]
        announced_sequence, announced_index = self.announced_output_order.get(pending_id, (None, None))
