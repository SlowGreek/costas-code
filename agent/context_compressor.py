...
class ContextCompressor(SummaryDispatchMixin, MicroCompactionMixin, ContextEngine):
    ...
    def update_from_response(self, usage: Dict[str, Any]):
        ...
        # Legacy fork callers can still pair their exact request estimate. New
        # runtime phases use durable usage_anchor fingerprints instead; never
        # calibrate a usage-anchored figure a second time.
        if self.last_prompt_tokens > 0:
            if self.last_prompt_tokens < self.threshold_tokens:
                baseline = usage.get("_hermes_request_estimate_tokens", 0)
                if not baseline:
                    baseline = (self.last_compression_rough_tokens
                        if self.awaiting_real_usage_after_compression and self.last_compression_rough_tokens > 0
                        else self._pending_request_rough_tokens)
                if baseline > 0:
                    self.last_rough_tokens_when_real_prompt_fit = baseline
