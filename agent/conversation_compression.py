    ...
    try:
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Catalyst "
...
def _begin_compression_attempt(agent: Any, *, force: bool, defer_notification: bool) -> _Attempt:
    """Snapshot the compressor and reset signals without superseding an admitted owner.
    Ownership is claimed after the lease (or at the native Codex route).
    Signals are cleared at the VERY TOP, before codex/breaker
    ...
    # A contender is not a successor: claim only after durable lease admission.
    generation = 0
...
def _route_codex_compaction(
    ...
    attempt.generation = _claim_compressor_attempt(agent.context_compressor)
...
def compress_context(
    ...
    # Only an admitted owner can supersede another compressor attempt. A lock
    # loser returned above and cannot invalidate the live owner's summary.
    attempt.generation = _claim_compressor_attempt(agent.context_compressor)
