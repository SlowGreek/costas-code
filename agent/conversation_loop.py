...
def _apply_active_turn_redirect(agent: Any, messages: List[Dict[str, Any]], text: Any) -> None:
    ...
    checkpoint_parts = [_INTERRUPT_SCAFFOLD_MARKER, _REDIRECT_CONTINUATION_INSTRUCTION]
    if visible:
    ...
    correction = [{"type": "text", "text": prefix}, *text] if isinstance(text, list) else f"{prefix}\n\n{text}"
...
def run_conversation(
    ...
    """
    from agent.pending_user_input import current_inbox, UserInputInbox
    if getattr(agent, "_user_input_inbox", None) is None:
    ...
    try:
        result = _run_conversation_inner(
            ...
            moa_config=moa_config, persist_user_platform_id=persist_user_platform_id,
        )
    except Exception as exc:
