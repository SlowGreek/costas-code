...
def _exhausted_ttl(
    ...
    *failure_reason* is the classified semantics from
    ...
    if (
        ...
        #
        # Gated on failure_reason because providers ALSO return 403 for a
        # spending/key limit (OpenRouter "key limit exceeded", xAI spending
        # block). Those classify as billing and are genuine depletion — a 5min
        # retry just re-fails every 5 minutes — so they fall through to
        # upstream's billing ladder below and keep the full bench.
        #
        # Gated on sole_credential because upstream's ladder already shortens a
        # transient 403 to EXHAUSTED_TTL_SOLE_CREDENTIAL_SECONDS (60s) when
        # there is nothing to rotate to — strictly better than this 5min path
        # for the case where the only key is benched.
        ...
        *,
        ...
        with self._lock:
            ...
            if status_code == 403 and is_copilot_provider(self.provider):
                ...
                self._current_id = None
                # Upstream changed _select_unlocked to return
                # (entry, pending_refresh); unpack rather than treating the
                # tuple as an entry.
                next_entry, _pending_refresh = self._select_unlocked()
                ...
                if next_entry is not None and next_entry.id == entry.id:
                    # _available_entries also returns (available, pending_refresh)
                    # upstream — unpack before filtering.
                    _available, _ = self._available_entries(clear_expired=True)
                    alternatives = [
                        candidate
