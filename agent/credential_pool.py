        ...
        *,
        ...
        with self._lock:
            ...
            if status_code == 403 and is_copilot_provider(self.provider):
                # Authorization/policy is not depletion; retain healthy credentials.
                self._current_id = None
                next_entry, _pending = self._select_unlocked(refresh=False)
                if next_entry is not None and next_entry.id == entry.id:
                    available, _pending = self._available_entries(clear_expired=True, refresh=False)
                    alternatives = [candidate for candidate in available if candidate.id != entry.id]
                    if alternatives:
