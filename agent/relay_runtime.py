    ...
    def _initialize(self, relay: Any) -> bool:
        ...
        if dynamic_plugins:
            try:
                ...
                raise RuntimeError("Catalyst Relay dynamic plugin activation failed") from exc
    ...
    def _session_context(self, session: RelaySession, *, allow_closing: bool) -> contextvars.Context:
        ...
        with session.lock:
            if session.closing and not allow_closing:
                raise RuntimeError("Catalyst Relay session is closing")
            if session.context is None or session.handle is None:
                raise RuntimeError("Catalyst Relay session context is unavailable")
...
class RelaySessionCoordinator:
    ...
    def begin_turn(self, lease: ConversationLease, *, turn_id: str, task_id: str) -> RelayTurnContext:
        if lease.released:
            raise RuntimeError("Catalyst Relay conversation lease is released")
...
def _configured_plugin_inputs(relay: Any) -> tuple[dict[str, Any], list[Any]] | None:
    ...
    try:
        ...
        if "dynamic_plugins" in config:
            raise ValueError("Catalyst [[dynamic_plugins]] records are unsupported; use Relay [[plugins.dynamic]] records")
        ...
        raise _RelayPluginConfigurationLoadError(
            f"Catalyst Relay plugin configuration could not be loaded from {config_path}; continuing without Relay plugins"
        ) from exc
...
def run_in_session(
    ...
    **kwargs: Any,
    ...
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("Catalyst Relay runtime is unavailable")
    session = runtime.get_session(session_id)
    if session is None:
        session = runtime.ensure_session({"session_id": session_id})
    if session is None:
        raise RuntimeError("Catalyst Relay session is unavailable")
