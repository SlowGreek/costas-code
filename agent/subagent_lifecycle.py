...
_REQUEST_REJECTIONS: tuple[tuple[Callable[[Any], bool], str], ...] = (
    ...
    (lambda r: r.working_directory is not None,
     "working_directory is not supported because Catalyst delegates use isolated task environments."),
    (lambda r: bool(r.blocked_tools),
     "Per-tool blocking is not supported; use allowed_toolsets. Catalyst always blocks unsafe child tools."),
)
...
class SubagentLifecycleService:
    ...
    def launch(self, request: SubagentLaunchRequest) -> SubagentHandle:
        ...
        if parent is None:
            raise SubagentLifecycleError("No active Catalyst parent session is available.")
        ...
        if not subagent_id:
            raise SubagentLifecycleError("Catalyst failed to assign a child identity.")
        created = time.time()
        handle = SubagentHandle(
