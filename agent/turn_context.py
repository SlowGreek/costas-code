...
def substitute_api_content(api_msg: Dict[str, Any]) -> Optional[Any]:
    ...
    if sidecar and (
        isinstance(sidecar, str) and api_msg.get("role") in ("user", "assistant")
        or isinstance(sidecar, list) and api_msg.get("role") == "user"
    ):
...
def extract_api_content_sidecar(msg: Mapping[str, Any]) -> Optional[Any]:
    ...
    return v if isinstance(v, str) or (msg.get("role") == "user" and isinstance(v, list)) else None
...
def build_api_messages(
    ...
    payload_limit = getattr(agent, "max_request_payload_bytes", 0) or 0
    if payload_limit > 0:
        from agent.context_compressor import estimate_messages_payload_bytes
        if estimate_messages_payload_bytes(messages) > payload_limit:
            agent._try_strip_user_image_parts(messages, keep_newest=True)
    api_messages = []
    for idx, msg in enumerate(messages):
        ...
        _has_api_content = bool(_api_content) and (
            isinstance(_api_content, str) or (msg.get("role") == "user" and isinstance(_api_content, list))
        )
