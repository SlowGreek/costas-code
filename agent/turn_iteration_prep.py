...
def prepare_iteration(agent: Any,*, messages: Any, api_call_count: Any) -> IterationPrep:
    ...
    # User input is committed only after the complete preceding tool batch.
    from agent.message_metadata import append_message
    commit = getattr(agent, "_commit_pending_user_input", None)
    if callable(commit) and commit(messages):
...
def begin_iteration(
    ...
    from agent.conversation_loop import (
        _apply_active_turn_redirect, _review_input_budget_exhausted, _redirect_display_text
    )
    ...
    if _redirect_text:
        ...
        if isinstance(original_user_message, str):
            original_user_message = (
                f"{original_user_message}\n\n" f"User correction during the turn: {_redirect_display_text(_redirect_text)}"
            )
