...
def _recover_payload_too_large(st: _Recovery, _retry: TurnRetryState) -> OverflowVerdict:
    ...
    if new_bytes > 0 and new_bytes < original_bytes * 0.95:
        if len(messages) < original_len:
    ...
    stripped_tool_media = agent._try_strip_image_parts_from_tool_messages(st.messages, remember_model=False)
    if agent._try_strip_image_parts_from_tool_messages(st.api_messages, remember_model=False) or stripped_tool_media:
        agent._buffer_status(
    ...
    for keep_newest in (True, False):
        if agent._try_strip_user_image_parts(st.messages, keep_newest=keep_newest):
            agent._buffer_status("📐 Removed attached images to fit the request size limit; retrying...")
            _retry.restart_with_compressed_messages = True
            return st.done("break")
