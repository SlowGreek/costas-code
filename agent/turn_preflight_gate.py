...
def run_preflight_gate(
    ...
    from agent.model_metadata import calibrated_request_pressure_tokens
    request_pressure_tokens = calibrated_request_pressure_tokens(
        _compressor, request_pressure_tokens,
        provider_anchored=bool(getattr(agent, "_request_pressure_anchored", False)),
    )
