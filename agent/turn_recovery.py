...
def _print_anthropic_401_diagnostics(agent: Any, key: Any) -> None:
    ...
    _plines(
        agent,
        "   Troubleshooting:",
        f"     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Catalyst-managed OAuth/setup tokens",
...
def recover_after_classification(
    ...
    from agent.conversation_loop import _is_copilot_provider, _is_recoverable_copilot_forbidden, _copilot_error_haystack
    if (_is_copilot_provider(agent) and not _retry.copilot_forbidden_retry_attempted
        and _is_recoverable_copilot_forbidden(status_code, _copilot_error_haystack(api_error))):
        _retry.copilot_forbidden_retry_attempted = True
        if agent._try_refresh_copilot_client_credentials():
            agent._buffer_vprint("🔐 Copilot token re-exchanged after 403. Retrying request...")
            return True, recovered_with_pool
