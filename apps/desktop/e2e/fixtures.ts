 ...
 *
 * Pins `display.busy_input_mode: interrupt`. Costas Code ships `steer` as the
 * product default, where a prompt arriving mid-turn steers the live turn in
 * place instead of starting its own — correct for users, wrong for a harness
 * that counts completions. `RealSessionBuilder` submits turns back-to-back
 * with zero think-time and waits for one `message.complete` per prompt, so
 * under `steer` the second prompt is absorbed into turn 1, no second
 * completion is emitted, and the builder hangs for its full 60s timeout.
 * Measured against the real gateway (4 turns, mock provider): `steer` hangs
 * forever on turn 2; `interrupt` completes all 4 in 8.0s.
 *
 * Tests that want to exercise the default policy should override this via
 * `extraDisplayConfig`.
 *
