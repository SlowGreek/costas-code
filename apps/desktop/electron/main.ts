...
import { registerPeepsVoiceAuthIpc } from './peeps-voice-auth-ipc'
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    // Consume the repair flags: this pass refetches the installer, subsequent
...
ipcMain.handle('hermes:bootstrap:repair', async () => {
  ...
  try {
    ...
    rememberLog(`[bootstrap] repair: backend teardown failed: ${error?.message ?? error}`)
  }
...
ipcMain.handle('hermes:saveClipboardImage', async () => {
  ...
  if (IS_WSL) {
    ...
    if (png) {
      ...
      return shrunk.buffer.length ? writeComposerImage(shrunk.buffer, shrunk.ext) : writeComposerImage(png, '.png')
    }
  }
