...
test('Windows provisioning and validation scripts are fixed CurrentUser-only non-elevating contracts', () => {
  ...
  assert.doesNotMatch(WINDOWS_ACL_SCRIPT, /Set-Acl/)
  assert.match(WINDOWS_ACL_SCRIPT, /GetAccessControl.*AccessControlSections.*Access/s)
  assert.match(WINDOWS_ACL_SCRIPT, /SetAccessControl/)
