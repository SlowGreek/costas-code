...
test('shallow clone with differing SHAs reports an uncountable update, never a fake 1', () => {
  // Upstream dropped the `hasMergeBase` input (shouldCountCommits keys on
  // `isShallow` alone) AND replaced the old numeric `1` sentinel with null,
  // because the UI rendered that 1 as a literal "1 change included" even when
  // the real distance was far larger. A full clone with an empty countStr is
  // simply 0; the uncountable case is the shallow one.
  assert.equal(
    resolveBehindCount({
      countStr: '',
      currentSha: 'local',
