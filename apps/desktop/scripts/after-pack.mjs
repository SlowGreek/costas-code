...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Catalyst'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Catalyst.exe keeps the stock Electron icon`)
  }
}
