...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Costas Code'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Costas Code.exe keeps the stock Electron icon`)
  }
}
