...
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Costas Code.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Costas Code'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Costas Code.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Costas Code'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Costas-Code-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Costas-Code-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Costas-Code-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Costas-Code-${version}-${os}-${arch}.${ext}'
