...
import { $goalJudgingBySession } from '@/store/composer-status'
import {
...
import { type ConfirmState, useSessionValue } from './session-control-utils'
...
export const SessionControlGoalSection = memo(function SessionControlGoalSection({
  ...
  const judging = Boolean(useSessionValue($goalJudgingBySession, sessionId))
  const blocked = goal.status === 'blocked' || goal.last_verdict === 'blocked'
        ...
        <ContextMenuTrigger asChild>
          <div data-slot="session-control-goal">
            <StatusSection
              ...
              icon={
                judging ? (
                  <GlyphSpinner ariaLabel="judging…" className={iconClass} spinner="braille" />
                ) : (
                  <Codicon className={iconClass} name="target" size="0.8rem" />
                )
              }
              label={headerLabel}
            >
              <div className="space-y-1.5 px-1 py-1">
                ...
                {/* Native blocked reasons must stay visible, not be truncated. */}
                {goal.blocked_reason && (
                  <div className="text-[0.7rem] text-destructive break-words">{goal.blocked_reason}</div>
                )}
                {/* Optional reasons */}
