...
import { $goalJudgingBySession, setGoalJudging } from '@/store/composer-status'
...
describe('ComposerStatusStack session-control UI', () => {
  ...
  it('keeps judging visible in the structured goal row until the verdict arrives', () => {
    $sessionControlBySession.set({ [SID]: mockEntry() })
    renderStack()
    act(() => setGoalJudging(SID, true))
    expect(screen.getByLabelText('judging…')).toBeTruthy()
    act(() => setGoalJudging(SID, false))
    expect(screen.queryByLabelText('judging…')).toBeNull()
  })

  it('shows the native blocked reason and offers resume', async () => {
    $sessionControlBySession.set({
      [SID]: mockEntry({
        snapshot: sampleSnapshot({
          goal: sampleGoal({ status: 'blocked', blocked_reason: 'Need operator evidence' })
        })
      })
    })
    renderStack()
    expect(screen.getByText('Need operator evidence')).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: /goal actions/i }))
    expect(await screen.findByRole('menuitem', { name: /resume goal/i })).toBeTruthy()
  })
