          ...
          {(() => {
            const leadNode = reorderable ? (
              ...
              <SidebarRowLead
                className={cn(
                  'overflow-hidden',
                  // FORK: a branched row renders the tree stem AND the dot in a
                  // lead sized (size-3.5) for one glyph, so the two get crushed
                  // together ("nested dots are smooshed"). Widen to fit only
                  // here; unbranched rows keep the shared fixed metric so every
                  // other row still aligns.
