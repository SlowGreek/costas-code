...
export function handleInputRequestEvent(ctx: GatewayEventContext): boolean {
  ...
  if (event.type === 'clarify.expire') {
    ...
    // returned empty. Without this the card kept offering choices that could
    // no longer resolve anything — the request is gone from the gateway's
    // `_pending`. Drop the live request so the row settles into the "Skipped"
    // card (which keeps the options actionable as a drafted follow-up)
    // instead of pretending to still be answerable.
    ...
    // Only when THIS session's live request was the one that expired. A stale
    // expire (already superseded or answered) is a no-op, and must not drop
    // the sidebar indicator on a session still blocked on a newer question.
