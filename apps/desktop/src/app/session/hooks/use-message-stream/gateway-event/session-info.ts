...
export function handleSessionInfoEvent(ctx: GatewayEventContext): boolean {
  ...
  if (event.type === 'session.usage' || event.type === 'usage.update') {
    ...
    // `usage.update` is the same payload emitted after every tool call, so
    // the context gauge tracks a long turn live rather than freezing at its
    // pre-turn value; both share this write shape.
