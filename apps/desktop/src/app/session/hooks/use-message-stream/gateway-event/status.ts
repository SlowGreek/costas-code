...
import { markSessionErrored, setTurnStartedAt } from '@/store/session'
...
export function handleStatusEvent(ctx: GatewayEventContext): boolean {
  ...
  if (event.type === 'status.update') {
    if (sessionId && payload?.kind === 'compacting') {
      ...
      // The turn is done painting but the goal is still deciding: the judge
      // (and, on DONE, the verifier) each cost an auxiliary LLM round-trip.
      // Mark the goal row as judging so the app doesn't look idle through
      // that gap — the verdict clears it moments later.
