...
describe('usePromptActions /title', () => {
  ...
  describe('usePromptActions smart workspace preparation', () => {
    ...
    it('routes a later mutating turn to the synchronously selected child session', async () => {
      ...
      // Upstream added an entry-time ownership proof (#64789/#65328): a
      // selected stored session must positively map to the active runtime in
      // runtimeIdByStoredSessionIdRef, or submit drops the runtime id and
      // takes the resume path. Seed both bindings so this test exercises the
      // routing it is actually about rather than the ownership guard.
      const runtimeIdByStoredSessionIdRef = {
        current: new Map([
          ['stored-parent', 'rt-parent'],
          ['stored-child', 'rt-child']
        ])
      }
...
describe('usePromptActions redirectPrompt', () => {
  ...
  // Upstream retired the insert-before-the-active-reply contract in favour of
  // appendMidTurnUserMessage, which SEALS the live assistant bubble and appends
  // the correction after it (rewind.ts: "retires the old
  // insert-before-the-active-reply contract whose lastAssistantIndex fallback
  // could splice the bubble mid-thread when the stream id was missing or
...
describe('usePromptActions stale-closure session routing', () => {
  ...
  it('resubmits an unchanged latest user turn when no assistant response exists', async () => {
    ...
    // Upstream #91302 drops the client ordinal once a durable row id is
    // present (renderer vs gateway ordinal spaces trip the 4030 cross-check),
    // so the row id alone is the truncation address here.
    expect(requestGateway).toHaveBeenCalledWith(
      'prompt.submit',
      expect.objectContaining({
        confirm_empty_truncate: true,
