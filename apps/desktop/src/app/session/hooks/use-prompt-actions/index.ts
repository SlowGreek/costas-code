...
export function usePromptActions({
    ...
    (
      ...
      updateSessionState(
        ...
        state => {
          ...
          // Upstream's appendMidTurnUserMessage (above) supersedes the fork's
          // insertBeforeActiveReply splice: it seals the live stream so
          // post-redirect deltas continue BELOW the correction, which is the
          // same ordering the fork's index maths was reaching for (and fixes
          // the #73793 / #83151 mis-splices the fork version still had).
  ...
  const submitPromptText = useSubmitPrompt({
    ...
    getRouteToken,
    prepareSessionForPrompt,
    // Window dispatcher keeps the exact owner and turn lease through completion.
    requestGateway,
    runtimeIdByStoredSessionIdRef,
    resumeStoredSession,
    selectedStoredSessionIdRef,
