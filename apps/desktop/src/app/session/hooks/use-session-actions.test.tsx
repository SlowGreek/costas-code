...
describe('desktop branch creation idempotency', () => {
  ...
  it('coalesces duplicate stored-session branch attempts onto one backend child', async () => {
    ...
    await waitFor(() =>
      expect(requestGateway.mock.calls.filter(([method]) => method === 'session.branch')).toHaveLength(1)
    )
  ...
  it('does not coalesce two same-id parents that live on different connections', async () => {
    ...
    await act(async () => {
      second = actions!.branchStoredSession('parent')
      await waitFor(() =>
        expect(routedCreate.mock.calls.filter(([, , method]) => method === 'session.branch')).toHaveLength(2)
      )
    })
    ...
    const creates = routedCreate.mock.calls.filter(([, , method]) => method === 'session.branch')
...
describe('branchStoredSession desktop source tagging', () => {
  ...
  it('routes a connection-tagged parent branch through its owning connection', async () => {
    ...
    expect(requestGatewayForAgent).toHaveBeenCalledWith(
      ...
      'default',
      'session.branch',
      expect.objectContaining({ stored_session_id: 'stored-parent', omit_messages: true })
    )
    // ...and never the ambient socket, which may serve a different machine.
    expect(ambientRequest).not.toHaveBeenCalledWith('session.branch', expect.anything())
  })
  ...
  it('keeps an untagged parent branch on the ambient socket', async () => {
    ...
    expect(createParams).toMatchObject({ stored_session_id: 'stored-parent', omit_messages: true })
