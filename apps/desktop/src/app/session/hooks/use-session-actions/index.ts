...
import { $projectScope, cleanupManagedWorktreePath, resolveNewSessionCwd } from '@/store/projects'
...
function branchCreateKey({
  ...
  return JSON.stringify({
    ...
    sourceSessionId,
    sourceStoredSessionId: sourceStoredSessionId ?? null
  })
}
  ...
  const createBackendSessionForSend = useCallback(
    async (preview: string | null = null): Promise<string | null> => {
      ...
      try {
        ...
        const params = await desktopSessionCreateParams(requestedCwd, capturedRoute)
    ...
    async (
      ...
      try {
        ...
        if (sourceStoredSessionId && !responseBranchMessages.length) {
          try {
            const persistedTail = await getLatestSessionMessages(routedSessionId, ownerRoute ?? profile)
