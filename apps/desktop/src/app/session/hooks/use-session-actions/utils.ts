...
function persistedUserMatchesOptimistic(authoritative: ChatMessage, optimistic: ChatMessage): boolean {
  ...
  // directive line while the optimistic row keeps it in `attachmentRefs`, so a
  // raw text compare always mismatched and duplicated the bubble. Reduce both
  // sides to their visible text first.
  if (textWithoutImageRefs(authoritativeText).trim() === textWithoutImageRefs(optimisticText).trim()) {
    return true
  }
  ...
  // TRAILING form: the DB appends generated-image hints / [screenshot]
  // placeholders after the caption.
  ...
  return nextMessages.map(message => {
    ...
    if (message.role === 'user' && persistedUserMatchesOptimistic(message, previous)) {
      return {
        ...
        // Carry the cached previews forward ONLY when the authoritative row has
        // none of its own. A resumed row that already knows its attachments is
        // the authority; overwriting it with a stale local cache regresses to
        // the previous session's image paths.
        attachmentRefs: message.attachmentRefs?.length ? message.attachmentRefs : previous.attachmentRefs
      }
    }
