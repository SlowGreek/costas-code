...
export const zh: Translations = {
  ...
  sessionImport: {
    title: '从其他应用继续',
    subtitle: '将对话导入 Catalyst，接着上次的进度继续。',
    action: '导入会话',
    readingFrom: '读取自',
  ...
  settings: {
    ...
    plugins: {
      ...
      agent: {
        title: '智能体插件',
        blurb:
          '你安装到 Catalyst 后端的插件——工具、技能、MCP 服务器、钩子和斜杠命令。「便携」插件是 Agent Plugins 标准包（技能 + MCP 组合，也可在其他智能体中使用）。开关在新会话中生效。',
        appliesTo: '应用于：',
        empty: '尚未安装智能体插件。',
    ...
    appearance: {
      ...
      tipsTitle: '应用内提示',
      tipsDesc: '指向应用某处的小气泡：空闲时偶尔出现，需要时 Catalyst 也会给你一条。关掉一条就不再出现。',
      tipsReset: (count: number) => `恢复 ${count} 条已关闭的提示`,
      toursTitle: '引导导览',
      toursDesc: '让 Catalyst 带你熟悉应用：调暗界面并逐步高亮每个位置。',
    ...
    about: {
      ...
      bundleOutOfSyncDesc:
        'Catalyst 运行时已更新,但桌面应用本身仍是旧构建——在应用更新之前,新的界面功能(如 Bot Mode)不会显示。请运行下方的更新以重新构建应用。如果此警告仍未消除,请从最新的桌面安装程序重新安装。',
      bundleOutOfSyncAction: '获取安装程序',
      bundleSwapPending: '重启以完成更新',
      bundleSwapPendingDesc: '更新后的应用已安装完成，只需重启 Catalyst 即可加载新版本。聊天记录和设置不会受到影响。',
      bundleSwapPendingAction: '重启 Catalyst',
      updates: '更新',
      checkNow: '立即检查',
      checking: '检查中…',
      seeWhatsNew: '查看新增内容',
    ...
    connections: {
      title: '已注册网关',
      intro: '管理本机以及通过远程、SSH 或 Hermes Cloud 连接可访问的每个 Catalyst 网关。',
      stagedNote:
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'Catalyst 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 Catalyst 后端时，可以使用远程网关。网关连接属于本机级设置；profile 是从所连接的网关中发现的。',
    ...
    localModels: {
      ...
      runtimeInstalledDetail: (tag, backend) => `构建 ${tag}，${backend} 后端。Catalyst 会为您启动并管理服务器。`,
      ...
      upToDateDetail: (tag, backend) => `正在运行 llama.cpp ${tag}（${backend}）——Catalyst 提供的最新构建。`,
    ...
    toolsets: {
      ...
      browserRealProfile: {
        ...
        prompt: {
          title: '让网站保持登录状态',
          body: '让 Catalyst 使用默认浏览器配置文件的快照进行浏览，网站打开时即已登录。',
  ...
  profiles: {
    ...
    remoteOverride: {
      ...
      description: '此配置档案中的会话将在你指定的远程 Catalyst 上运行，而不是这台电脑。',
      urlLabel: '远程地址',
  ...
  preview: {
    ...
    web: {
      ...
      blankPageBody: '在上方输入地址开始浏览，或让 Catalyst 打开一个页面。',
  ...
  tips: {
    ...
    items: {
      ...
      skills: {
        title: '教一次就够',
        text: '技能是一组说明文件，Catalyst 会在需要时自行加载。'
      },
      messaging: {
        title: '离开电脑也能用',
      ...
      profiles: {
        title: '配置文件彼此独立',
        text: '每个都是独立的 Catalyst — 自己的密钥、记忆和会话。'
      },
      'composer-mentions': {
        title: '附件与命令',
