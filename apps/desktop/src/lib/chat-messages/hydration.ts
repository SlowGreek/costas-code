...
export function toChatMessages(messages: SessionMessage[]): ChatMessage[] {
  ...
  messages.forEach((message, index) => {
    ...
    // Realtime voice: one HUMAN turn can span several provider response
    // continuations, each persisted as its own assistant row. The backend
    // stamps them with a shared `semantic_turn_id` so the transcript can fold
    // them back into a single bubble instead of showing the reply in pieces.
    ...
    // Fold a continuation into the bubble that opened this semantic turn —
    // but only while it is still the newest turn. A later user message means
    // the voice turn is closed, so a stale id must start a fresh bubble
    // rather than append to history.
    const semanticIndex =
      displayRole === 'assistant' && normalizedSemanticTurnId
        ? result.findLastIndex(
            candidate => candidate.role === 'assistant' && candidate.semanticTurnId === normalizedSemanticTurnId
          )
        : -1
    ...
    if (displayRole === 'assistant' && normalizedSemanticTurnId && semanticIndex >= 0 && !newerUserExists) {
      ...
      target.realtimeRowIds = [...(target.realtimeRowIds ?? []), ...(rowId !== undefined ? [rowId] : [])]
