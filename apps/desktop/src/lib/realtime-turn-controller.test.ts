...
describe('RealtimeTurnController', () => {
  ...
  it('continues with a required action when terminal verification rejects completion', async () => {
    const sent: Record<string, unknown>[] = []

    const verifyTerminal = vi.fn(async () => ({
      context: 'The response only promised the walkthrough; the requested explanation has not happened.',
      kind: 'continue_once' as const,
    ...
    controller.beginTurn('Walk me through the data flow step by step.')
    controller.responseCreated('response-1')
    controller.assistantTranscriptDone('response-1', 'Sure, let’s walk through it.')
    controller.functionCallDone(call('response-1', 'call-finish', 'finish_turn'))

    expect(await controller.responseDone('response-1')).toEqual({ continued: true, settled: false })
    expect(verifyTerminal).toHaveBeenCalledOnce()
    expect(controller.activeTurn()).not.toBeNull()

    const output = sent.find(event => event.type === 'conversation.item.create')

    expect(JSON.parse((output?.item as { output: string }).output)).toMatchObject({
      accepted: false,
      error: 'Completion proposal rejected'
    })
    expect(sent.at(-1)).toMatchObject({
      response: {
        instructions: expect.stringMatching(/requested explanation has not happened/i),
  ...
  it('drops a late terminal verdict after barge-in', async () => {
    const verification = deferred<{ context: string; kind: 'continue_once'; toolChoice: 'required' }>()
    const send = vi.fn()
    ...
    const controller = createRealtimeTurnController({
      ...
      laneFor: () => 'terminal',
      send,
      verifyTerminal: () => verification.promise
    })

    controller.beginTurn('Explain every stage.')
    controller.responseCreated('response-1')
    controller.functionCallDone(call('response-1', 'call-finish', 'finish_turn'))
    ...
    verification.resolve({
      context: 'The explanation is incomplete.',
      kind: 'continue_once',
      toolChoice: 'required'
    })

    await expect(completing).resolves.toEqual({ continued: false, settled: false })
