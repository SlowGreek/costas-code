...
interface PeepsVoiceAuthBridge {
  ...
  start: (
    id: string,
    flow: { authority: string; clientId: string; redirectUri: string; scope: string; state: string }
  ) => Promise<boolean>
  wait: (id: string, timeoutMs: number) => Promise<null | string>
}
  ...
  return {
    async complete(
      ...
      try {
        ...
        ensureCurrent()
        let peepsToken = await Promise.race([
