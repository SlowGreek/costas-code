...
export function rearmPendingPrompts(
  ...
  for (const prompt of prompts) {
    ...
    setClarifyRequest({
      choices: choices.length > 0 ? choices : null,
      // Upstream added multiSelect to ClarifyRequest. A re-armed prompt has no
      // payload for it (the announcing event is gone), so default to false —
      // single-select is the shape every pre-multiSelect clarify had.
      multiSelect: Boolean((payload as { multi_select?: boolean })?.multi_select),
      question,
      requestId,
      sessionId
    })
