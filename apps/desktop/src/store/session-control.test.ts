...
describe('session-control store', () => {
  ...
  it('accepts a native blocked goal without losing its structured controls', () => {
    const raw = {
      ...FULL_SNAPSHOT,
      goal: { ...FULL_SNAPSHOT.goal!, status: 'blocked', blocked_reason: 'Need operator evidence' }
    }

    expect(parseSessionControlSnapshot(raw)?.goal).toMatchObject({
      status: 'blocked',
      blocked_reason: 'Need operator evidence'
    })
  })
