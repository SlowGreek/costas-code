...
export type SessionControlGoalStatus = 'active' | 'blocked' | 'done' | 'paused'
...
const GOAL_STATUSES = new Set<SessionControlGoalStatus>(['active', 'blocked', 'done', 'paused'])
...
function parseGoal(value: unknown): SessionControlGoal | null {
  ...
  if (
    ...
    !hasOptionalStrings(value, ['paused_reason', 'blocked_reason', 'last_reason']) ||
  ...
  if (hasOwn(value, 'blocked_reason')) {
    goal.blocked_reason = value.blocked_reason as string
  }
