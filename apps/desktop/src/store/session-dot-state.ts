...
import { $erroredSessionIds, $sessions, $unreadFinishedSessionIds, lineageAliases } from './session'
import {
  $attentionSessionIds,
  $draftSessionIds,
  $sessionStates,
...
import { $subagentsBySession, $subagentWaitingSessionIds, activeSubagentCount } from './subagents'
...
export type SessionDotState =
  | 'background'
  | 'draft'
  // Fork: the turn ended in an error and the user hasn't opened it since.
  | 'error'
  | 'idle'
  | 'needs-input'
  | 'stalled'
  // Fork: the turn is running AND waiting on delegated subagent work — strictly
  // more specific than plain `working`, so it claims above it.
  | 'subagent'
  | 'unread'
  | 'working'
...
export const showsRunningArc = (state: SessionDotState): boolean =>
  state === 'stalled' || state === 'working' || state === 'subagent'
...
export const sessionStatusBucket = (state: SessionDotState = 'idle'): SessionStatusBucket =>
  state === 'stalled' || state === 'background' || state === 'subagent'
    ? 'working'
    : state === 'error'
      ? 'unread'
      : state
...
export const $sessionDotStateById = computed(
  ...
  (
    ...
    // Fork: a running turn that is waiting on delegated work. REFINES working
    // (same as stalled) rather than rivalling it, so it can never invent a
    // running session from a stale subagent hint.
    for (const id of subagentWaiting) {
      for (const alias of lineageAliases(id, sessions)) {
        if (next[alias] === 'working' || next[alias] === 'stalled') {
          next[alias] = 'subagent'
        }
      }
    }

    // Fork: a failed turn the user hasn't opened. Above every "running" state
    // (the turn is over) and below needs-input (a blocking prompt is actionable
    // right now; an error is already done).
    claim(errored, 'error')
