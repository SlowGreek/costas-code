...
class GatewayGoalsMixin:
    ...
    async def _post_turn_goal_continuation(
        ...
        decision = await self._run_in_executor_with_context(
            lambda: mgr.evaluate_after_turn(
                ...
                active_delegations=_active_deleg, recent_evidence=recent_evidence,
            ),
        )
    ...
    async def _run_post_turn_hooks(
        ...
        if isinstance(captured := getattr(event, "_goal_agent_result", None), dict):
            agent_result = captured
        try:
            session_entry = await self.async_session_store.get_or_create_session(
        ...
        eligible, _ = self._post_turn_goal_should_judge(
            agent_result if isinstance(agent_result, dict) else final_text)
        if eligible:
        ...
        for label, hook in hooks:
            try:
                ...
                if label == "goal continuation":
                    ...
                    kwargs["recent_evidence"] = extract_recent_tool_evidence(
    ...
    def _post_turn_goal_should_judge(agent_result):
        ...
        if isinstance(agent_result, dict):
            ...
            failed = bool(agent_result.get("failed")) or agent_result.get("completed") is False
        elif isinstance(agent_result, str):
