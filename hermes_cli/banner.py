...
def _github_compare_behind(current_rev: str, target_rev: str) -> Optional[int]:
    ...
    if not (_is_full_sha(current_rev) and _is_full_sha(target_rev)):
        return None
    url = (
        # Fork identity: derive the compare endpoint from the fork's own repo
        # constant. Upstream hardcodes nousresearch/hermes-agent here, which in
        # this fork would compare our SHAs against a repo that has never seen
        # them — every probe 404s and the banner silently loses its count.
        f"https://api.github.com/repos/{_OFFICIAL_REPO_CANONICAL.split('github.com/', 1)[-1]}/"
...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    try:
        ...
        # for the same reason. The explicit refspec keeps the
        # ``origin/<UPDATE_BRANCH>`` tracking ref current so the
        # ``HEAD..origin/<UPDATE_BRANCH>`` count below is accurate; the shallow
        # path compares against FETCH_HEAD, which a scoped fetch also updates.
        fetch_args = [
            "git",
            "fetch",
            "origin",
