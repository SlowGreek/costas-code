...
class CLICommandsMixin:
    ...
    def _print_session_diff(self, cwd: str, stat_only: bool):
        ...
        if result.get("empty") or (not stat and not diff):
            return print("  No changes — Catalyst hasn't edited any files here yet.")
        if stat:
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        result = dispatch_goal_command(
            ...
            live_steer=lambda text: bool(
                getattr(self, "_agent_running", False) and self.agent is not None
                and self.agent.steer(text)),
