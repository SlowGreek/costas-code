...
class CLILoopsMixin:
    ...
    def _maybe_continue_goal_after_turn(self) -> None:
        ...
        decision = mgr.evaluate_after_turn(
            ...
            active_delegations=_active_deleg, recent_evidence=self._recent_goal_evidence(),
            status_callback=lambda kind, text=None: (
                _cprint(f"  {_DIM}⋯ {text or 'judging goal'}{_RST}")
                if kind == "judging" else None),
        )
    ...
    def _recent_goal_evidence(self, max_items: int = 6, max_chars: int = 1500) -> list:
        ...
        Feeds the single completion judge with real artifacts (tool
    ...
    def _maybe_wake_parked_goal(self) -> None:
        ...
        try:
            ...
            if mgr is None:
                return
            if not self._pending_input.empty() or getattr(self, "_agent_running", False):
                return
            prompt = mgr.poll_wake()
            if prompt:
                self._pending_input.put(prompt)
