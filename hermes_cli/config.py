...
def validate_env_var_name_for_write(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "or Catalyst runtime location and security policy (HERMES_HOME, HERMES_YOLO_MODE, ...) "
...
def show_config():
    ...
    print(color("│              ⚕ Catalyst Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
...
def _coerce_config_set_value(key: str, value: str) -> Any:
    ...
    if isinstance(_default_value_for_key(key), list):
...
def _print_unknown_key_notice(key: str, suggestion: Optional[str]) -> None:
    print(color(
        ...
        "but Catalyst may not read it.", Colors.YELLOW))
    if suggestion:
