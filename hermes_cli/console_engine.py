"""Safe Catalyst Console command engine."""
...
_BLOCKED_PAIRS = {
    ("config", "edit"): "`config edit` opens an editor and is not available in Catalyst Console.",
    ("mcp", "serve"): "`mcp serve` starts a server and is not available in Catalyst Console.",
    ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Catalyst Console.",
    ("skills", "config"): "`skills config` is interactive and is not available in Catalyst Console.",
    ("skills", "publish"): "`skills publish` is not available in Catalyst Console.",
    ("portal", "login"): "`portal login` is interactive and is not available in Catalyst Console.",
    ("portal", "open"): "`portal open` opens a browser and is not available in Catalyst Console.",
    ("kanban", "tail"): "`kanban tail` streams output and is not available in Catalyst Console.",
    ("kanban", "watch"): "`kanban watch` streams output and is not available in Catalyst Console.",
    ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Catalyst Console.",
    ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Catalyst Console.",
    ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Catalyst Console.",
    ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Catalyst Console.",
    ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Catalyst Console.",
    ("kanban", "gc"): "`kanban gc` is not available in Catalyst Console.",
    ("sessions", "delete"): "`sessions delete` and `sessions prune` are not available in Catalyst Console.",
    ("sessions", "prune"): "`sessions delete` and `sessions prune` are not available in Catalyst Console.",
}


class HermesConsoleEngine:
    """Curated line-command executor for Catalyst Console."""
    ...
    def help_text(self, subject: str | None = None) -> str:
        ...
        lines = ["Catalyst Console", "", "Supported commands:"]
    ...
    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Catalyst Console."
        if first in _BLOCKED_TOP:
            return f"`hermes {first}` is not available in Catalyst Console."
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError(
                "auth add --type api-key requires --api-key in Catalyst Console.")
...
_BUILTIN_COMMANDS = (
    ...
    (("config", "migrate"), "config migrate", "Update config with new options.", _config_migrate,
     "Update Catalyst configuration with missing defaults?"),
...
def run_console_repl(
    *, stdin=None, stdout=None, stderr=None, interactive: bool | None = None) -> int:
    ...
    say("Catalyst Console. Type `help` for commands, `exit` to quit.")
    while True:
