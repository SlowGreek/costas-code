...
def print_legacy_unit_warning() -> None:
    ...
    if not legacy:
        return
    print_warning("Legacy Catalyst gateway unit(s) detected from an older install:")
...
def remove_legacy_hermes_units(interactive: bool = True, dry_run: bool = False) -> tuple[int, list[Path]]:
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy Catalyst gateway units found.")
        return 0, []

    print()
    print("Legacy Catalyst gateway unit(s) found:")
    for name, path, is_system in legacy:
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Catalyst install")
    return True
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Catalyst install")
    return True
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Catalyst install")
    else:
        print("⚠ Service definition is stale relative to the current Catalyst install")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error("Refusing to run the Catalyst gateway as root inside the official Docker image.")
    print(
        "  The image entrypoint normally drops privileges to the 'hermes' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/hermes/docker/entrypoint.sh before the Catalyst command."
    )
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Catalyst Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
