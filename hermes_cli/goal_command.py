...
def _steer(mgr, arg, live_steer=None):
    if not mgr.has_goal():
        return GoalCommandResult("No active goal to steer. Set one with /goal <text>.")
    if not arg or arg.lower() == "show":
        return GoalCommandResult(mgr.render_steers())
    if arg.lower() == "clear":
        count = mgr.clear_steers()
        return GoalCommandResult(f"✓ Cleared {count} steer{'s' if count != 1 else ''}."
                                 if count else "No steers to clear.")
    added = mgr.add_steer(arg)
    live = False
    if live_steer is not None:
        try:
            live = bool(live_steer(arg))
        except Exception:
            logger.debug("live goal steer failed; durable correction retained", exc_info=True)
    when = ("Applied to the running turn and to every step after it." if live
            else "Applied from the next step on.")
    return GoalCommandResult(f"↪ Steering the goal: {added}\n{when}")
...
def is_goal_control(arg: str) -> bool:
    ...
    return normalized in _EXACT_HANDLERS or normalized.split(None, 1)[0] in {"wait", "gate", "steer"}
...
def dispatch_goal_command(
    ...
    live_steer: Callable[[str], bool] | None = None,
) -> GoalCommandResult:
    ...
    try:
        ...
        if verb == "steer":
            prefix = "/goal steer"
            return _steer(mgr, rest, live_steer)
        if verb == "wait":
            prefix = "/goal wait"
