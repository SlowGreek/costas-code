...
def _desktop_packaged_executable_in(release_dir: Path) -> Optional[Path]:
    ...
    *release_dir* is electron-builder's ``directories.output`` — the live ``apps/desktop/release`` or a
    ...
    if sys.platform == "darwin":
        candidates = [p for product in ("Catalyst", "Costas Code", "Hermes")
                      for p in release_dir.glob(f"mac*/{product}.app/Contents/MacOS/{product}")]
    elif sys.platform == "win32":
        candidates = [
            release_dir / d / f"{product}.exe" for d in ("win-unpacked", "win-ia32-unpacked", "win-arm64-unpacked")
            for product in ("Catalyst", "Costas Code", "Hermes")
        ]
    else:
        candidates = [
            release_dir / d / n for d in ("linux-unpacked", "linux-arm64-unpacked") for n in ("Catalyst", "Costas Code", "hermes", "Hermes")
        ]
