...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        _say("", "  Auxiliary models — side-task routing", "",
             ...
             "  Catalyst only falls back to a lightweight backend (OpenRouter,",
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (is_claude_code_token_valid(creds) or bool(creds.get("refreshToken"))):
            ...
            print(f"    Catalyst will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _build_provider_picker_rows(config: dict, active: str, provider_labels: dict[str, str],
    ...
    _visible_slugs = [slug for slug in _visible_slugs if provider_is_allowed(slug, config)]
