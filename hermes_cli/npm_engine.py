...
def upgrade_managed_npm(npm: str, npm_range: str, *, prefix: Path, quiet: bool = False) -> bool:
    ...
    """
    if not quiet:
        print(f"→ Upgrading Catalyst-managed npm to satisfy {npm_range}…", flush=True)
    ...
    if managed_node_tree_in_use():
        if not quiet:
            print(
                "  ⚠ deferred: the Catalyst-managed Node.js tree is in use by a "
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(
        ...
        f"  Resolved npm: {npm}\n"
        "  Catalyst could not provision its own Node.js runtime and never\n"
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    ...
    if not quiet:
        print(
            "→ Provisioning a Catalyst-managed Node.js runtime "
