...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    _info("The interactive wizard cannot be used here.", None,
          "Configure Catalyst using environment variables or config commands:",
...
def _apply_default_agent_settings(config: dict):
    ...
    _info("  Max iterations: 150", "  Tool progress: all", "  Compression threshold: 0.85",
...
def _run_setup_section(config: dict, section: str) -> None:
    ...
    if entry is None:
        ...
        return
    label, func = entry
    _print_banner(f"│     ⚕ Catalyst Setup — {label:<34s} │")
...
def _run_setup_wizard_impl(args):
    ...
    _print_banner("│             ⚕ Catalyst Setup Wizard                │",
                  "├─────────────────────────────────────────────────────────┤",
                  "│  Let's configure your Catalyst installation.       │",
    ...
    if is_existing:
        ...
        print_success("You already have Catalyst configured.")
