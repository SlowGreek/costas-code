...
def _cmd_update_check(branch: str = "costas-code", *, branch_explicit: bool = False):
    ...
    fetch_result = None
    if _upstream_syncable_branch(branch) and _git_run(git_cmd, ["remote", "get-url", "upstream"]).returncode == 0:
        print("→ Fetching from upstream...")
        fetch_result = _git_run(git_cmd, ["fetch"] + depth_args + ["upstream", _m()._tracking_refspec("upstream", branch)], network=True)
    if fetch_result is not None and fetch_result.returncode == 0:
        compare_branch = f"upstream/{branch}"
    else:
        print("→ Fetching from origin...")
        fetch_result = _git_run(git_cmd, ["fetch"] + depth_args + ["origin", _m()._tracking_refspec("origin", branch)], network=True)
...
def _repair_current_checkout(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, desktop_dir,
    ...
    if runtime_repaired is not None and not _m()._is_windows():
        ...
        print(
            "  Restart each of them to pick up the repaired runtime, then remove the parked venv"
            + (f": {runtime_repaired.backup_venv}" if runtime_repaired.backup_venv is not None else ".")
        )
...
def _prepare_checkout_for_update(
    ...
    if commit_count == 0 and is_fork and _upstream_syncable_branch(branch):
        ...
        upstream_checked = _m()._sync_with_upstream_if_needed(
            git_cmd, _m().PROJECT_ROOT, branch, assume_yes=assume_yes, input_fn=gw_input_fn)
...
def _apply_pulled_update(
    ...
    if is_fork and _upstream_syncable_branch(branch):
        _m()._sync_with_upstream_if_needed(
            git_cmd, _m().PROJECT_ROOT, branch, assume_yes=opts.assume_yes, input_fn=opts.gw_input_fn)
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        fetch_result = _git_run(git_cmd, ["fetch", "origin", _m()._tracking_refspec("origin", branch)], network=True)
        if fetch_result.returncode != 0:
            _print_fetch_failure(fetch_result.stderr)
            sys.exit(1)
