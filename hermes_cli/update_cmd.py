...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    ...
    if branch != _m().COSTAS_UPDATE_BRANCH:
        print(
        ...
        print(
            ...
            f"--branch {branch}`, or update against "
            f"{_m().COSTAS_UPDATE_BRANCH} with `hermes update`."
        )
        _m().sys.exit(1)
    _abort_zip_update_if_dirty_tree()
    zip_url = (
        f"https://github.com/SlowGreek/costas-code/archive/refs/heads/{branch}.zip"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Catalyst behaves unexpectedly.")
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Catalyst couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Catalyst couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Catalyst behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Catalyst couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Catalyst couldn't drop "
...
# Catalyst is its own distribution: `hermes update` tracks the Catalyst repo,
# not upstream Nous. Pointing these at NousResearch would make every Catalyst
# install look like a "fork" to the update flow and sync from the wrong remote.
OFFICIAL_REPO_URLS = {
    "https://github.com/SlowGreek/costas-code.git",
    "git@github.com:SlowGreek/costas-code.git",
    "https://github.com/SlowGreek/costas-code",
...
def _sync_fork_with_upstream(
    ...
    """Attempt to push the updated distribution branch to origin (sync fork).
    ...
    try:
        result = subprocess.run(
            git_cmd + ["push", "origin", branch, "--force-with-lease"],
            cwd=cwd,
            capture_output=True,
            text=True, encoding="utf-8", errors="replace",
        )
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    print()
    print(f"→ Restoring {len(missing)} Catalyst Tools dependency set(s)...")
    restored: list[str] = []
    ...
    if branch == _m().COSTAS_UPDATE_BRANCH:
        ...
        if has_upstream_remote:
            ...
            fetch_result = subprocess.run(
                git_cmd + ["fetch"] + depth_args + ["upstream", _m()._tracking_refspec("upstream", branch)],
                cwd=_m().PROJECT_ROOT,
                capture_output=True,
                text=True, encoding="utf-8", errors="replace",
            )
        if fetch_result is not None and fetch_result.returncode == 0:
            ...
            fetch_result = subprocess.run(
                git_cmd + ["fetch"] + depth_args + ["origin", _m()._tracking_refspec("origin", branch)],
                cwd=_m().PROJECT_ROOT,
                capture_output=True,
                text=True, encoding="utf-8", errors="replace",
            )
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    print("→ Stopping Windows gateway process(es) before updating Catalyst...")
    try:
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        fetch_result = subprocess.run(
            git_cmd + ["fetch", "origin", _m()._tracking_refspec("origin", branch)],
            cwd=_m().PROJECT_ROOT,
            capture_output=True,
            text=True, encoding="utf-8", errors="replace",
        )
        ...
        if commit_count == 0:
            ...
            if handed_off_sync or not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Catalyst windows/gateways and re-run: hermes update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Catalyst gateways, Desktop backends, or other "
                    "long-lived processes still use the previous runtime."
                )
                # FORK: name the parked venv. The repair leaves the old venv on
                # disk and the user cannot clean it up if we never say where it
                # is. Upstream's line drops the path; keep it appended here.
                print(
                    "  Restart each of them to pick up the repaired runtime, "
                    "then remove the parked venv"
                    + (
        ...
        if sys.platform == "darwin" and had_desktop_app_before_update:
            print()
            print(
                "  ℹ macOS: if Catalyst re-prompts for permissions you already "
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("⚠ Legacy Catalyst gateway unit(s) detected:")
