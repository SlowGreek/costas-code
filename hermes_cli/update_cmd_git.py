...
_BAR = "=" * 68
_UPSTREAM_ADD_CMD = "git remote add upstream https://github.com/SlowGreek/costas-code.git"
...
def _sync_fork_with_upstream(git_cmd: list[str], cwd: Path, branch: str | None = None) -> bool:
    """Push updated main to origin (sync fork); True on success."""
    from hermes_cli.main_install_repair import COSTAS_UPDATE_BRANCH
    branch = branch or COSTAS_UPDATE_BRANCH
    return _git_ok(git_cmd, ["push", "origin", branch, "--force-with-lease"], cwd, network=True)
...
def _offer_upstream_remote(git_cmd: list[str], cwd: Path, *, assume_yes: bool, input_fn) -> bool:
    ...
    print(
        "\nℹ Your fork is not tracking the official Catalyst repository.\n"
        "  This means you may miss updates from SlowGreek/costas-code.\n"
    )
    ...
    if not _add_upstream_remote(git_cmd, cwd):
        ...
        return False
    print("  ✓ Added upstream: https://github.com/SlowGreek/costas-code.git")
    return True


def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path, branch: str | None = None, *, assume_yes: bool = False, input_fn=None) -> bool:
    ...
    from hermes_cli.main_install_repair import COSTAS_UPDATE_BRANCH, _tracking_refspec
    branch = branch or COSTAS_UPDATE_BRANCH
    ...
    print("\n→ Fetching upstream...")
    try:
        subprocess.run(git_cmd + ["fetch", "upstream", _tracking_refspec("upstream", branch), "--quiet"], cwd=cwd, capture_output=True, check=True, **_no_prompt_git_kwargs())
    ...
    if origin_ahead > 0:
        print(
            ...
            f"  If you want to merge upstream changes, run:\n    git pull upstream {branch}"
        )
        return True
    if upstream_ahead == 0:
    ...
    try:
        subprocess.run(git_cmd + ["pull", "--ff-only", "upstream", branch], cwd=cwd, check=True, **_no_prompt_git_kwargs())
    except subprocess.CalledProcessError:
