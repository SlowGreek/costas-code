...
def _print_verified_update_completion(message: str) -> bool:
    ...
    print("  Rebuild the Catalyst venv with a uv-managed Python, restart Catalyst, then verify with `hermes doctor`.")
    return False
    ...
    if node_failures or not desktop_build_ok or not sqlite_runtime_ok:
        ...
        if not sqlite_runtime_ok:
            print(
                ...
                "update` again; if SQLite is unchanged, rebuild the Catalyst venv "
                "with a uv-managed Python, restart Catalyst, then verify with "
                "`hermes doctor`."
            )
    else:
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    try:
        ...
        print(
            ...
            "handles (or restart Catalyst) and retry."
        )
        return False
