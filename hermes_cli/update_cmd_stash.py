...
def _reject_unsafe_stash_restore(
    ...
    from hermes_cli.update_cmd import _git_untracked_paths
    print()
    print("✗ Restored local changes made the Catalyst agent unexecutable.")
...
def _drop_restored_stash(git_cmd: list[str], cwd: Path, stash_ref: str) -> None:
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but Catalyst couldn't find the stash entry to drop.")
        print(_STASH_LEFT_IN_PLACE)
        _print_stash_cleanup_guidance(stash_ref)
        return
    drop = _git_run(git_cmd, ["stash", "drop", stash_selector], cwd)
    if drop.returncode != 0:
        print("⚠ Local changes were restored, but Catalyst couldn't drop the saved stash entry.")
...
def _discard_stashed_changes(git_cmd: list[str], cwd: Path, stash_ref: str) -> bool:
    ...
    drop = _git_run(git_cmd, ["stash", "drop", stash_selector], cwd)
    if drop.returncode != 0:
        print("⚠ Configured to discard local changes, but Catalyst couldn't drop the saved stash entry.")
