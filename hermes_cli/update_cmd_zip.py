...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    ...
    if branch != _m().COSTAS_UPDATE_BRANCH:
        ...
        print(
            ...
            f"--branch {branch}`, or update against costas-code with `hermes update`."
        )
        _m().sys.exit(1)
    _abort_zip_update_if_dirty_tree()
    _download_and_swap_zip(branch, f"https://github.com/SlowGreek/costas-code/archive/refs/heads/{branch}.zip")
