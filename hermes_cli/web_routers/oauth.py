...
async def _start_copilot_device_code(profile: Optional[str]) -> Dict[str, Any]:
    ...
    from hermes_cli.web_server_oauth import _copilot_poller
    device_data = await asyncio.to_thread(copilot_request_device_code)
    expires_in = int(device_data.get("expires_in") or 900)
    interval = int(device_data["interval"])
    return _device_session_started(
        "copilot", profile, _copilot_poller,
        dict(device_code=str(device_data["device_code"]), interval=interval,
             expires_at=time.time() + expires_in),
        str(device_data["user_code"]), str(device_data["verification_uri"]),
        expires_in, interval)


_DEVICE_CODE_STARTERS = {
    "copilot": _start_copilot_device_code,
