...
def _recent_upstream_commits(n: int = 20) -> List[Dict[str, Any]]:
    ...
    non-zero. Pinning to the distribution branch keeps both views consistent.

    Best-effort: returns [] if the ref or git is unavailable. Never raises.
    """
    try:
        out = subprocess.run(
            [
...
def _report_port_in_use(host: str, port: int) -> None:
    ...
    print(
        ...
        "'hermes serve' / 'hermes dashboard' backend or the Catalyst gateway. "
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if headless:
                ...
                print(f"  Catalyst backend listening on {host}:{actual_port}", flush=True)
            else:
                print(f"  Catalyst Web UI → http://{host}:{actual_port}")
