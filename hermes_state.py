...
def _default_db_path() -> Path:
    ...
    1. A live ``HERMES_HOME`` that differs from the import-time snapshot wins.
       The FORK contract (tests/test_session_db_isolation.py): pytest's autouse
       fixture redirects HERMES_HOME *after* this module is imported, so an
       import-time constant would send ``SessionDB()`` at the developer's real
       ``~/.hermes/state.db``.
    2. Otherwise a deliberately re-pointed ``DEFAULT_DB_PATH`` (differs from the
       import-time snapshot — upstream's test escape hatch) wins.
    3. Otherwise resolve ``get_hermes_home()`` fresh.
    """
    live = get_hermes_home() / "state.db"

    if live != _IMPORT_DEFAULT_DB_PATH:
        return live
...
class SessionDB(
    ...
    def append_message(
        ...
        sidecar for prompt-cache-stable replay: strings are stored as sent and
        structured content is tagged JSON so it round-trips without colliding
        with ordinary JSON-looking text. Lone surrogates are scrubbed because
        sqlite3 cannot bind them and the conversation loop scrubs them from
        outgoing payloads too.
        """
        ...
        # cannot bind list/dict parameters directly. The provider-replay
        # sidecar uses the same tagged encoding so structured wire content can
        # survive reload without being confused with a plain JSON-looking string.
        ...
        for row in rows:
            ...
            if msg.get("tool_calls"):
                try:
                    ...
                    msg["tool_calls"] = []
            # display_metadata is stored as a JSON TEXT column. Every read path
            # must decode it; serving `SELECT *` verbatim reaches the desktop as
            # a JSON *string* where the schema promises an object, and
            # `'task_count' in message.display_metadata` then throws and fails
            # the whole resume.
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        for row in rows:
            ...
            # verbatim — no sanitize_context, no strip — because replay
            # substitutes it for content to keep the provider prompt-cache
