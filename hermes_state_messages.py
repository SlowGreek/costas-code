...
class SessionMessagesMixin:
        ...
        *keep_reasoning* False NULLs every reasoning column). ``platform_message_id`` falls back to
        ...
        return (session_id, role, self._encode_content(msg.get("content")), msg.get("tool_call_id"),
            ...
            *(self._reasoning_json_text(_reasoning(k))
            ...
            self._encode_content(msg.get("api_content")) if isinstance(msg.get("api_content"), (str, list)) else None,
    ...
    def append_message(
        ...
        the platform's own id. ``api_content``: fidelity sidecar, the exact string or structured
        content sent to the API when it differed from ``content``. Lists use tagged JSON;
        plain JSON-looking text remains text, and lone surrogates are scrubbed."""
    ...
    def restore_rewound(self, session_id: str, since_message_id: int) -> int:
        """Mark rewound (not compaction-archived) messages active again.
