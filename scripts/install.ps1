...
$RepoUrlSsh = "git@github.com:SlowGreek/costas-code.git"
$RepoUrlHttps = "https://github.com/SlowGreek/costas-code.git"
$PythonVersion = "3.11"
...
function Resolve-AvailablePythonVersion {
    ...
    foreach ($ver in $candidates) {
        ...
        try {
            ...
            $startInfo.Arguments = "python find $(Get-UvPythonRequest $ver) --managed-python --no-config"
...
function Test-Python {
    ...
    try {
        ...
        $ErrorActionPreference = "Continue"
        $uvOutput = & $UvCmd python install (Get-UvPythonRequest $PythonVersion) --no-bin --no-registry --no-config 2>&1
    ...
    foreach ($fallbackVer in $PythonFallbackVersions) {
        try {
            ...
            $ErrorActionPreference = "Continue"
            & $UvCmd python install (Get-UvPythonRequest $fallbackVer) --no-bin --no-registry --no-config 2>&1 | Out-Null
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/SlowGreek/costas-code/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/SlowGreek/costas-code/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/SlowGreek/costas-code/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
