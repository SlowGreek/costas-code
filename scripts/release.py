...
def update_version_files(semver: str, calver_date: str):
    ...
    # re-locking leaves the two disagreeing. CI runs `uv sync --locked`, which
    # refuses a stale lockfile — that fails dependency install on EVERY test
    # slice before a single test runs, plus the dedicated `uv lock --check`
    # job. Re-lock here so the bump commit is self-consistent.
    if shutil.which("uv"):
        result = subprocess.run(
            ["uv", "lock"], cwd=str(REPO_ROOT), capture_output=True, text=True
        )
        if result.returncode != 0:
            print(f"  ⚠ uv lock failed: {result.stderr.strip()[:200]}")
            print("    Run `uv lock` manually before pushing, or CI will fail.")
    else:
        print("  ⚠ uv not found — run `uv lock` manually before pushing.")
...
def generate_changelog(commits, tag_name, semver, repo_url=None,
    ...
    lines.append(f"# Catalyst v{semver} ({tag_name})")
    lines.append("")
...
def main():
    ...
    if args.publish:
        ...
        if args.bump:
            ...
            # Commit version bump
            # uv.lock is regenerated by update_version_files (it pins the
            # project's own version); stage it or the bump commit ships a
            # lockfile that disagrees with pyproject and breaks CI install.
            add_files = [str(VERSION_FILE), str(PYPROJECT_FILE)]
            for extra in (REPO_ROOT / "uv.lock", REPO_ROOT / "apps" / "desktop" / "package.json"):
                if extra.exists():
                    add_files.append(str(extra))
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Catalyst v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Catalyst v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Catalyst v{new_version} ({calver_date})' "
