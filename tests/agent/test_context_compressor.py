...
class TestCalibratedPreflightTokens:
    ...
    def test_keeps_raw_estimate_once_rough_growth_outruns_the_measured_request(self, compressor):
        ...
        # Deferral now projects real usage instead of tolerating a fixed rough
        # growth band (upstream): 113K->125K is 12K of rough growth, so
        # projected real is 58K+12K = 70K, far under the 100K threshold —
        # deferring is correct. The old fixed-tolerance rule compacted here at
        # 70% of the real window, which is the CJK/replay-blob churn upstream
        # removed. Calibration (asserted below) still keeps the raw estimate.
        assert compressor.should_defer_preflight_to_real_usage(125_000) is True
