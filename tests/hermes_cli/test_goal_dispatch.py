...
def _surface(surface, mgr, monkeypatch, prompts=None):
    ...
    server._sessions[mgr.session_id] = {
        'session_key': mgr.session_id, 'history_lock': threading.RLock(),
        'running': False, 'pending_goal': None,
    }
    def execute(arg):
...
def test_surface_goal_state_matches_cli(surface, command, monkeypatch):
    ...
    for name in ('cli', surface):
        ...
        if state:
            ...
            state = asdict(state)
            # Compare wait duration, not wall-clock instants from separate
            # surface invocations. Catalyst adds a deadline to PID waits.
            if state['waiting_until']:
                state['waiting_until'] = round(state['waiting_until'] - state['waiting_since'])
