...
class TestGatherBackgroundProcessesOwnership:
    def test_only_the_owning_sessions_processes_are_seen(self, monkeypatch):
        ...
        # The fork refuses an unscoped global snapshot; explicit owner scope
        # below retains upstream's parent-vs-child filtering contract.
        assert goals.gather_background_processes() == []
...
class TestBlockedVerdict:
    ...
    def test_blocked_verdict_preserves_native_blocked_state_instead_of_done(self, hermes_home):
        from unittest.mock import patch
        from hermes_cli.goals import GoalManager
        ...
        assert decision["should_continue"] is False
        assert "the repo does not exist" in decision["message"]
        assert mgr.state is not None
        assert mgr.state.status == "blocked"
        assert mgr.state.blocked_reason == "the repo does not exist"
