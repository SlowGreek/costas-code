...
def _shallow_git(head_sha, fetch_head_sha):
    def fake_run(cmd, **kwargs):
        if cmd[:4] == ["git", "remote", "get-url", "origin"]:
            return MagicMock(
                returncode=0,
                # Fork: the counted path only engages for the fork's OWN
                # official remote; an upstream URL takes the
                # non-official early return instead. Derive it from the
                # module constant so a rebrand can't silently skip this.
                stdout=f"https://{banner._OFFICIAL_REPO_CANONICAL}.git\n",
            )
