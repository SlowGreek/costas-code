...
class TestDeadRetryCode:
    ...
    def test_no_unreachable_max_retries_after_backoff(self):
        from agent import conversation_loop, turn_api_error
        # Retry handling is now an extracted phase. Guard reachability rather
        # than freezing the number of guard statements in its former owner.
        for module in (conversation_loop, turn_api_error):
                    ...
                    for index, statement in enumerate(value[:-1]):
                        if isinstance(statement, ast.Raise):
                            assert not any(
                                ...
                                and "retry_count >= max_retries" in ast.unparse(later.test)
                                for later in value[index + 1:]
                            ), f"unreachable retry guard in {module.__name__}"
