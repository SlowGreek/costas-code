...
def test_try_refresh_copilot_client_credentials_rebuilds_client(monkeypatch):
    ...
    # Fork divergence (02c3cfb94e "align auth and inference with the
    # official runtime"): Hermes upstream sends "vscode-chat"; Catalyst
    # sends the official runtime's id. Sourced from the constant so a
    # future rename does not silently re-hardcode a stale value.
    from hermes_cli.copilot_auth import COPILOT_INTEGRATION_ID

    assert rebuilt["kwargs"]["default_headers"]["Copilot-Integration-Id"] == COPILOT_INTEGRATION_ID
    assert isinstance(agent.client, _RebuiltClient)
