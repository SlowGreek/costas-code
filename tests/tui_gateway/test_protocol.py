...
def test_session_branch_persists_branched_from_marker(server, monkeypatch):
    ...
    class _DB:
        ...
        def append_messages_batch(self, session_key, messages, **_kwargs):
            # Upstream replaced per-row appends with a bounded-chunk batch
            # (#23254). Fan out to this stub's per-row recorder so the
            # existing assertions keep observing the same rows.
            for _m in messages:
                self.append_message(session_key=session_key, **_m)
...
def test_session_redirect_without_images_is_plain_text(server, monkeypatch):
    ...
    sid = "redir-plain"
    server._sessions[sid] = {
        "session_key": "20260101_000000_plain",
        "agent": _Agent(),
        "history": [],
        "history_lock": threading.Lock(),
        "running": True,
    ...
    resp = server.handle_request(
        {"id": "r2", "method": "session.redirect", "params": {"session_id": sid, "text": "just words"}}
    )

    assert "error" not in resp, resp
    assert captured["payload"] == "just words"
