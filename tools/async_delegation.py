...
def _persist_dispatch(record: Dict[str, Any]) -> None:
    ...
    task_payload = {
        key: record.get(key)
        for key in ("goal", "goals", "context", "toolsets", "role", "model", "is_batch", "task_indexes", "suppress_completion_delivery", *_ROUTING_KEYS)
        if key in record}
...
def _prune_durable_records() -> None:
    ...
    with _DB_LOCK, _transaction() as conn:
        conn.execute(
            "DELETE FROM async_delegations WHERE delivery_state IN ('delivered','suppressed') AND updated_at < ?", (cutoff,))
    ...
    with _DB_LOCK, _transaction() as conn:
        ...
        for row in rows:
            ...
            conn.execute("""UPDATE async_delegations SET state='unknown', completed_at=?,
                   ...
                   WHERE delegation_id=?""", (now, now, json.dumps(event), json.dumps(result), "suppressed" if task.get("suppress_completion_delivery") else "pending", delegation_id))
            recovered += 1
    return recovered
...
def dispatch_async_delegation(
    *, goal: str, context: Optional[str], toolsets: Optional[List[str]], role: str, model: Optional[str],
    ...
    handle = _dispatch(
        suppress_completion_delivery=suppress_completion_delivery, completion_callback=completion_callback,
