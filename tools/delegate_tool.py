...
from tools.delegate_tool_config import (  # noqa: F401
    ...
    _get_allowed_models, _parse_model_spec, _resolve_task_model_overrides, _describe_batch_models,
...
def _build_children(
    ...
    live_deleg_id: Optional[str], live_writers: list, task_creds: Optional[dict] = None,
    ...
    for i, t in enumerate(task_list):
        child_creds = (task_creds or {}).get(i, creds)
        overrides = {
...
def delegate_task(
    ...
    model: Optional[str] = None, suppress_completion_delivery: bool = False,
    ...
    if tasks is None and model is not None:
        task_list[0]["model"] = model
    task_creds, task_creds_error = _resolve_task_model_overrides(task_list, routing_cfg, parent_agent)
    if task_creds_error:
    ...
    batch = _Batch(
        ...
        task_creds=task_creds, suppress_completion_delivery=suppress_completion_delivery,
        reject_if_async_capacity=reject_if_async_capacity, completion_callback=completion_callback,
    )
...
def _build_top_level_description(*, independent_completions=None) -> str:
    ...
    return _DESCRIPTION_HEAD.format(delivery=delivery) + restrictions_rule + _build_model_selection_note()
...
registry.register(
    ...
    handler=lambda args, **kw: delegate_task(
        ...
        max_iterations=args.get("max_iterations"), role=args.get("role"), model=args.get("model"),
