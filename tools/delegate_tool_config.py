...
def _resolve_child_runtime(
    ...
    if (override_api_mode is None and effective_model != getattr(parent_agent, "model", None)
        and (str(effective_provider or "").lower() in {"copilot", "github-copilot", "github"}
             or "githubcopilot.com" in str(effective_base_url or "").lower())):
        try:
            from hermes_cli.models import copilot_model_api_mode
            effective_api_mode = copilot_model_api_mode(effective_model, api_key=override_api_key or parent_api_key)
        except Exception as exc:
            logger.debug("Could not derive Copilot api_mode for child model %r: %s", effective_model, exc)
