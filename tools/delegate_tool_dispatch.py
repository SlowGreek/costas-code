...
def _units_of(batch: _Batch) -> List[_Batch]:
    ...
    if batch.suppress_completion_delivery or not _get_independent_completions():
        return [batch]
...
def _dispatch_unit(unit: _Batch, unit_id: Optional[str], slot_key: Optional[str], routing: dict) -> dict:
    ...
    from tools.delegate_tool_config import _describe_batch_models
    ...
    return dispatch_async_delegation_batch(
        ...
        role=unit.top_role, model=_describe_batch_models(unit.task_list, unit.task_creds, unit.creds),
        suppress_completion_delivery=unit.suppress_completion_delivery, completion_callback=unit.completion_callback,
...
def _dispatch_background(batch: _Batch) -> str:
    ...
    if wake_sid is None and batch.suppress_completion_delivery:
        wake_sid = str(getattr(batch.parent_agent, "session_id", "") or "")
    if wake_sid is None:
    ...
    for k, unit in enumerate(units):
        ...
        if dispatch.get("status") == "dispatched":
            ...
            continue
        if not dispatched and batch.reject_if_async_capacity:
