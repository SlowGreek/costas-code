...
class ComputeHost:
    ...
    def handle_frame(self, frame: dict[str, Any]) -> None:
        ...
        elif kind == "control" and frame.get("route_name") == "session.input" and (frame.get("params") or {}).get("images"):
            self._executor.submit(self._handle_control, frame)  # never block Stop on image enrichment
        else:
    ...
    def _control_ack(self, server: Any, frame: dict[str, Any], session: dict) -> dict:
        ...
        if route_name in {"session.input", "session.input.status"}:
            response = server._methods[route_name](
                frame.get("request_id"), {**(frame.get("params") or {}), "session_id": sid})
            inbox = session.get("user_input_inbox")
            return {"rpc_response": response, "user_inputs": inbox.snapshot() if inbox else [],
