...
def _load_durable_truncation_history(
    ...
    ``repair_alternation=False`` yields VERBATIM durable rows for identity
    lookup. That is not a model-replay read: alternation repair can merge a
    display-only user marker (for example ``model_switch``) with the following
    real prompt and retain only the marker's row id, and a later edit must
    still be able to address the real prompt by its own durable id.
    """
...
def _resolve_truncate_row_id(session: dict, history: list, target_row_id: int):
    ...
    # Identity lookup, not a model-replay read: alternation repair can merge a
    # display-only user marker (e.g. model_switch) with the following real
    # prompt and keep only the marker's row id, so a later edit could no longer
    # address the real prompt. Read the rows VERBATIM.
...
def _(rid, params: dict) -> dict:
    ...
    with session["history_lock"]:
        ...
        if (
            ...
            # Upstream's helper is the default: it keeps a compaction handoff
            # carrier at the new head, which is the ONLY surviving
            # representation of already-compacted turns — a plain slice would
            # drop it.
            #
            # The fork's `target_history_index` covers a case the helper cannot
            # address: a durable row persisted before an interrupted turn that
            # was never appended to the live history (see "rerun interrupted
