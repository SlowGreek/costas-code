...
def _(rid, params: dict) -> dict:
    with _profile_db(params) as db:
        ...
        try:
            ...
            # their own source.
            from tui_gateway.server import _NON_HUMAN_SESSION_SOURCES

            # Union of upstream's deny-list and the fork's: upstream drops
            # "kanban", the fork drops "subagent" (its resume picker was
            # being buried by sub-agent fan-out rows). Both are noise in a
            # human resume picker; dropping either regresses one side.
            deny = _NON_HUMAN_SESSION_SOURCES | {"kanban"}
        ...
        if is_truthy_value(params.get("lazy", False)):
            ...
            return _ok(
                rid,
                {
                    ...
                    "info": _lazy_resume_info(cwd, profile=profile, session_key=target),
                    "inflight": None,
...
def _(rid, params: dict) -> dict:
    ...
    with _session_db(session) as db:
        ...
        def _visible_branch_history(messages, *, keep_tool_shape: bool = False):
            """Project a branch's copied history.

            ``keep_tool_shape`` distinguishes the two callers:

            * The in-memory session history is the MODEL projection — a tool
              result and the (often empty-content) assistant turn that called
              it are structural, and dropping them leaves the branch replaying
              an unanswered call.
            * The persisted display projection is what a human reads; tool
              plumbing is deliberately not part of it.
            """
            visible = []
            for message in messages or []:
                ...
                if role not in allowed:
                    continue
                # A tool result, and the assistant turn that called it, are
                # structural — not prose. Filtering them on empty text broke
                # the call/result pairing in a branch: the assistant's
                # tool_calls and the tool row both vanished, so the branched
                # conversation replayed an unanswered call. Keep any row that
                # carries tool-call shape even when its text is empty.
                has_tool_shape = keep_tool_shape and bool(
                    message.get("tool_calls")
                    or message.get("tool_call_id")
                    or message.get("tool_name")
                )
                if role == "tool" and not has_tool_shape:
                    continue
                if (
                    not has_tool_shape
                    and not _coerce_message_text(message.get("content")).strip()
                ):
                    continue
        ...
        if not history:
            history = _visible_branch_history(in_memory_history, keep_tool_shape=True)
        if not history:
                ...
                [
                    {
                        ...
                        # Tool-call shape. Without these a branch keeps the
                        # text but loses the pairing between a call and its
                        # result: the assistant's tool_calls vanish and the
                        # tool row can no longer be matched to the call that
                        # produced it. Upstream's batch rewrite dropped these
                        # three fields; the fork's fidelity test caught it.
