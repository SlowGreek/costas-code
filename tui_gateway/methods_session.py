...
_LISTING_DENY_SOURCES = frozenset({"kanban", "tool", "subagent"})
...
class _Resume:
    ...
    def info(self, cwd: str, overrides: dict) -> dict:
        return _lazy_resume_info(cwd, model=(overrides.get("model_override") or {}).get("model") or "",
                                 provider=overrides.get("provider_override") or "", profile=self.profile, session_key=self.target)
...
def _resume_lazy(ctx: _Resume) -> dict:
    ...
    return _resume_response(ctx, sid, record, info=_lazy_resume_info(cwd, profile=ctx.profile, session_key=ctx.target), display=display,
...
# ── session.branch ───────────────────────────────────────────────────
def _visible_branch_history(messages, *, keep_tool_shape: bool = False) -> list:
    """Keep visible display rows; model-history fallback also preserves call/result pairing."""
    visible = []
    for message in messages or []:
        if not isinstance(message, dict):
            continue
        role = message.get("role")
        allowed = {"user", "assistant", "tool"} if keep_tool_shape else {"user", "assistant"}
        has_tool_shape = keep_tool_shape and bool(
            message.get("tool_calls") or message.get("tool_call_id") or message.get("tool_name"))
        if role not in allowed or (role == "tool" and not has_tool_shape):
            continue
        if has_tool_shape or _coerce_message_text(message.get("content")).strip():
            visible.append(dict(message))
    return visible


def _branch_parent_cwd(session: dict):
    return session.get("cwd") if session.get("_cold_branch_parent") else _session_cwd(session)
    ...
    try:
        with _profile_build_scope(parent_home):
            ...
            _init_session(new_sid, new_key, agent, list(history), cols=session.get("cols", 80),
                          cwd=_branch_parent_cwd(session), session_db=branch_db, source=source, profile_home=parent_home,
...
def _branch_source_history(db, session: dict, old_key: str) -> list:
    ...
    return history or _visible_branch_history(in_memory_history, keep_tool_shape=True)
...
def _(rid, params: dict) -> dict:
    ...
    if stored_session_id:
        ...
        session = {"_cold_branch_parent": True, "session_key": found["id"], "history": [], "history_lock": threading.Lock(),
                   "cwd": found.get("cwd"), "source": found.get("source"),
    ...
    with _session_db(session) as db:
        ...
        try:
            ...
            _persist_branch(db, new_key, old_key, title, history, source=source, cwd=_branch_parent_cwd(session),
    ...
    return _ok(rid, {"session_id": new_sid, "stored_session_id": new_key, "title": title, "parent": old_key,
                     "message_count": len(history), "messages": [] if is_truthy_value(params.get("omit_messages", False)) else _history_to_messages(history),
...
def _apply_correction(rid, session: dict, verb: str, text: str, accepted_status: str, *, payload=None) -> dict:
    ...
    try:
        accepted = getattr(session["agent"], verb)(text if payload is None else payload)
    except Exception as exc:
...
def _correction_method(name: str, verb: str, accepted_status: str, supported, unsupported: str):
    """Register steer/redirect, preserving the shared correction bookkeeping and images."""
    @method(name)
    def _(rid, params: dict) -> dict:
        text = (params.get("text") or "").strip()
        images = list(dict.fromkeys(str(p) for p in (params.get("images") or []) if str(p).strip())) if verb == "redirect" else []
        ...
        if verb == "redirect":
            with session["history_lock"]:
                images = list(dict.fromkeys(images + list(session.get("attached_images") or [])))
        agent = session.get("agent")
        if verb == "redirect" and agent is None and session.get("running"):
            ...
            with session["history_lock"]:
                ...
                session["attached_images"] = [p for p in session.get("attached_images", []) if p not in images]
            session["last_active"] = time.time()
            return _ok(rid, {"status": "queued", "text": text})
        if not supported(agent):
            return _err(rid, 4010, unsupported)
        payload = _redirect_payload_with_images(agent, text, images) if images else None
        response = _apply_correction(rid, session, verb, text, accepted_status, payload=payload)
        if (response.get("result") or {}).get("status") == accepted_status and images:
            with session["history_lock"]:
                session["attached_images"] = [p for p in session.get("attached_images", []) if p not in images]
        return response
