...
def _cmd_goal(rid, params, session, name, arg):
    with _session_profile_runtime_scope(session or {}):
        ...
        with session["history_lock"]:
            pending = str(session.get("pending_goal") or "").strip()
            if normalized in {"pause", "clear", "stop", "done"}:
                session["pending_goal"] = None
            if not is_goal_control(arg) and session.get("running"):
                session["pending_goal"] = arg
                return _exec_out(rid, f"⏳ Goal queued — starts when this turn finishes: {arg.strip()}")
        result = dispatch_goal_command(
            mgr, arg, authorize_gate=lambda: None,
            last_user_message=goals.last_user_message_from_db(sid_key),
            live_steer=lambda text: bool(session.get("running") and session.get("agent")
                                          and session["agent"].steer(text)),
        )
        if result.error:
            return _err(rid, 4004, result.output)
        output = result.output
        if pending and normalized in {"", "status"}:
            output += f"\n⏳ Queued (starts at turn end): {pending}"
        elif pending and normalized in {"pause", "clear", "stop", "done"}:
            output += (f"\n⏸ Queued goal dropped: {pending}" if normalized == "pause"
                       else f"\n✓ Queued goal cleared: {pending}")
        if not result.prompt:
            return _exec_out(rid, output)
        payload = {"type": "send", "notice": output, "message": result.prompt}
...
_SLASH_BUILTINS = {
    ...
    "moa": _cmd_moa, "focus": _cmd_focus, "retry": _cmd_retry, "steer": _cmd_steer, "goal": _cmd_goal, "subgoal": _cmd_subgoal,
