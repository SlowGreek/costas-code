...
def _(rid, params: dict) -> dict:
    ...
    if name == "goal":
        ...
        # /goal steer <text> — a durable mid-loop course correction.
        #
        # Placed with the other CONTROL verbs (above the `session["running"]`
        # queue path) on purpose: steering while a turn runs is the entire
        # point, so it must never be swallowed by the pending_goal queue or
        # mistaken for new goal text. The correction is persisted on the goal,
        # so it survives the judge boundary and shapes every subsequent
        # continuation prompt — a steer delivered only to the live turn dies
        # when that turn ends and the loop resumes on the original wording.
        if lower == "steer" or lower.startswith("steer "):
            steer_arg = arg.strip()[len("steer"):].strip()
            if not mgr.has_goal():
                return _ok(
                    rid,
                    {
                        "type": "exec",
                        "output": (
                            "No active goal to steer. Set one with /goal <text>."
                        ),
                    },
                )
            if not steer_arg or steer_arg.lower() == "show":
                return _ok(rid, {"type": "exec", "output": mgr.render_steers()})
            ...
            try:
                ...
                return _ok(rid, {"type": "exec", "output": "No active goal to steer."})
            except ValueError as exc:
                return _err(rid, 4004, f"invalid steer: {exc}")
            # Persisting shapes every FUTURE continuation. If a turn is in
            # flight, also hand the text to the live agent so the correction
            # applies now instead of only after the judge boundary — the same
            # `agent.steer()` path the top-level /steer command uses. Purely
            # additive: a failure here still leaves the durable steer in place,
            # so the wording below only promises the live effect when it landed.
            live = False
            if session.get("running"):
                try:
                    agent = session.get("agent")
                    if agent is not None and hasattr(agent, "steer"):
                        live = bool(agent.steer(steer_arg))
                except Exception:
                    live = False
            now_line = (
                "Applied to the running turn and to every step after it."
                if live
                else "Applied from the next step on."
            )
            return _ok(
                rid,
                {
                    "type": "exec",
                    "output": (
                        f"↪ Steering the goal: {added}\n"
                        f"{now_line} /goal steer show · /goal steer clear"
                    ),
                },
            )
