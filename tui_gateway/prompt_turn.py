...
def _goal_followup_after_turn(
    ...
    if status == "interrupted":
        try:
            from hermes_cli.goals import GoalManager
            if session.get("session_key") and not session.get("_goal_user_preempted"):
                mgr = GoalManager(session_id=session["session_key"])
                if mgr.has_goal():
                    mgr.pause(reason="user-interrupted")
                    _emit("status.update", sid, {"kind": "goal", "text": mgr.status_line()})
        except Exception as exc:
            _hook_failure("goal interrupt-pause", exc)
        return None
    ...
    try:
        if session.get("session_key") and (goal_mgr := _active_goal_manager(session)) is not None:
            ...
            # the single completion judge — without
            ...
            decision = goal_mgr.evaluate_after_turn(
                ...
                active_delegations=_active_deleg, recent_evidence=_goal_evidence,
                status_callback=lambda kind, text=None: _status_update(
                    sid, "goal_judging" if kind == "judging" else "goal", text),
            )
...
def _finish_turn(sid: str, session: dict, st: _TurnRun) -> None:
    ...
    session.pop("_goal_user_preempted", None)
