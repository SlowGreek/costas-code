...
_LONG_HANDLERS = frozenset({"voice.realtime.token", "voice.realtime.peeps.complete",
    "voice.realtime.web_search", "voice.realtime.delegate_research", "workbench.visualize"}) | frozenset({
...
_AUTH_RESOLVED_ENDPOINT_PROVIDERS = {"copilot", "github", "github-copilot", "github-model", "github-models"}
...
def _shutdown_sessions() -> None:
    ...
    # A failed/partial import can register this atexit hook before split helpers bind.
    for name in ("_flush_sessions_before_exit", "_release_gateway_wake_owner"):
        with contextlib.suppress(Exception):
            globals()[name]()
    with _sessions_lock:
        sids = list(_sessions)
...
def _stored_session_runtime_overrides(row: dict | None) -> dict:
    ...
    if provider.lower() in _AUTH_RESOLVED_ENDPOINT_PROVIDERS:
        base_url = ""  # Current login, not persisted account metadata, owns the endpoint.
...
def _lazy_resume_info(cwd: str, *, model: str = "", provider: str = "", profile: str | None = None, session_key: str = "") -> dict:
    ...
    return {
        ...
        **({"usage": usage} if (usage := _stored_usage_for_session(session_key)) else {}),
    }
...
def _live_session_payload(
    ...
    for key, value in (("inflight", inflight), ("queued", queued),
        ...
        if value:
            payload[key] = value
    if pending_prompts := _session_pending_prompts(sid):
...
_PENDING_INPUT_COMMANDS: frozenset[str] = frozenset({
    "retry", "queue", "q", "steer", "plan", "goal", "subgoal", "loop", "proactive", "moa", "undo", "learn",
...
def _drain_pending_goal(rid, sid: str, session: dict) -> bool:
    ...
    with _session_profile_runtime_scope(session or {}):
        ...
        try:
            ...
            result = dispatch_goal_command(mgr, text, authorize_gate=lambda: None)
        ...
        try:
            ...
            if _session_uses_compute_host(session):
                ...
                # in-process runner.
                resp = _submit_prompt_to_compute_host(rid, sid, session, result.prompt)
                if resp.get("error"):
                ...
                _run_prompt_submit(rid, sid, session, result.prompt)
        except Exception as exc:
            print(
...
def _maybe_wake_parked_goal(sid: str, session: dict) -> None:
    ...
    with _session_profile_runtime_scope(session or {}):
        ...
        try:
            ...
            _emit("message.start", sid)
            if _session_uses_compute_host(session):
                response = _submit_prompt_to_compute_host(rid, sid, session, prompt)
                if response.get("error"):
                    raise RuntimeError((response["error"] or {}).get("message") or "goal wake refused")
            else:
...
from . import (  # noqa: E402
    methods_artifacts as _methods_artifacts, methods_realtime as _methods_realtime,
...
for _m in (
    ...
    _methods_session_control, _methods_subagents, _methods_artifacts, _methods_realtime, _methods_workbench):
