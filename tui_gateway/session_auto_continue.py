...
def _handle_busy_submit(rid, sid: str, session: dict, text: Any, transport: Any, queued: bool = False) -> dict | None:
    ...
    if session.get("_goal_continuation"):
        with session["history_lock"]:
            session["_goal_user_preempted"] = True
...
def _inflight_snapshot(session: dict) -> dict | None:
    ...
    if (inbox := session.get("user_input_inbox")) is not None and turn.get("turn_id"):
        snapshot.update(turn_id=turn["turn_id"], user_inputs=inbox.snapshot())
