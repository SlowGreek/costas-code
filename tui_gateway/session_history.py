...
def _start_inflight_turn(session: dict, text: Any) -> None:
    ...
    now = time.time()
    session["inflight_turn"] = {"turn_id": inbox.turn_id, "assistant": "", "started_at": now, "streaming": True, "updated_at": now, "user": _inflight_text(text)}
...
def _record_inflight_correction(session: dict, text: Any) -> None:
    ...
    if inbox is not None and inbox is getattr(session.get("agent"), "_user_input_inbox", None):
        if any(_inflight_text(item["content"]) == correction for item in inbox.snapshot()):
            return  # the identified receipt is authoritative; do not duplicate legacy bubbles
    turn = session.get("inflight_turn")
