...
```
  ...
  ─────────────────► │   (pre-agent, rough est.) │  Safety net for large sessions
                     ...
                     │ (real + anchored growth)  │  Normal context management
                     └──────────────────────────┘
```
...
anchors estimates to provider-reported usage and charges only newly appended messages.
...
|-----------|---------|-------|-------------|
| `threshold` | `0.85` | 0.0-1.0 | Compression triggers when usage-anchored request pressure ≥ `threshold × context_length` |
...
GitHub Copilot). With an explicitly lowered 50% trigger, compaction would fire at ~136K —
...
Intermediate context-pressure warnings have been removed (see the iteration-budget block in `agent/turn_iteration_prep.py`, which notes: "No intermediate pressure warnings — they caused models to 'give up' prematurely on complex tasks"). Compression fires when usage-anchored request pressure reaches the configured `compression.threshold` (default 85%) with no prior warning step; gateway session hygiene remains an independent 85% safety net before the agent starts.
