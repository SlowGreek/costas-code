...
`"queue"` mode prepares a separate follow-up turn. `"steer"` always waits for the next tool-result boundary and is the Catalyst default. `"interrupt"` responds sooner during model generation; a long foreground `terminal` command is handed to the background so the agent can read your message without killing the command. Use `/stop` to cancel the turn and its foreground work. Unknown values fall back to `"steer"`.
